(function ($) {
	...
	var visitorsChartWrapper = $('.visitors-chart-wrapper');
	if (visitorsChartWrapper.length) {
		var visitorsChart = $('#visitors-chart');
		var spinner = $('.visitors-chart-spinner');
		var url = visitorsChart.attr('data-url');

		(window.outerWidth <= 372) ? Chart.defaults.font.size = 4 : Chart.defaults.font.size = 8;
	    var options = {
	    	responsive: true,
	        scales: {
	            y: {
	    ...
	    $.ajax({
	        ...
	        success: function(response) {
	            if (response.status === 1) {
				    ...
				    var data = {
				        labels: $.map(response.labels, function(label, index) {
